...
export function useLeads({ search, statusFilter, ownerFilter, productFilter, pendingPaymentOnly, select, page = 1, pageSize = 25, fetchAll = false }: UseLeadsOptions & { fetchAll?: boolean; pendingPaymentOnly?: boolean } = {}) {
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<{ leads: Lead[]; count: number }> => {
      ...
      const defaultSelectQuery = tableName === 'leads'
        ? '*, sales_owner:profiles!leads_sales_owner_id_fkey(full_name)'
        : '*';

      const selectQuery = select || defaultSelectQuery;
